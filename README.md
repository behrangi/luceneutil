...
System-wide or uncore PMU events can be measured alongside the process-scoped events with `--perf-system-events`, for example `--perf-system-events 'arm_cmn_0/hnf_mc_reqs/,arm_cmn_0/hnf_mc_retries/'`. This option requires `--perf-control`. It starts a separate `perf stat -a` session with its own control/acknowledgement FIFOs, enables and disables it around the same exact measured phase, and writes its raw output to `perf-system.stat`. The existing process-scoped counters remain in `perf.stat`. System-wide perf may require elevated permissions or an appropriate `perf_event_paranoid` configuration.
